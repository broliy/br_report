...
RegisterCommand(Config.FeedbackClientCommand, function(source, args, rawCommand)
	if canFeedback then
		...
		exports['qb-Notify']:Alert("REPORT", "You can't report so quickly!", 10000, 'warning')
	end
end, false)
...
exports['qb-Notify']:Alert("REPORT", "New Report Some one needs help!", 20000, 'success')
		SendNUIMessage({
			action = "updateFeedback",
			FeedbackTable = FeedbackTable
		})
...
RegisterNUICallback("action", function(data)
	...
	if data.action == "newFeedback" then
		exports['qb-Notify']:Alert("REPORT", "Report successfully sent to the STAFF!", 20000, 'success')
		...
		if FeedbackTable[data.feedbackid] then
			if oneSync then
				...
				if playerOnline then
					...
					exports['qb-Notify']:Alert("REPORT", "That player is no longer in the server!", 20000, 'error')
				end
			end
		end
	elseif data.action == "concludeFeedback" then
		...
		if feedbackInfo then
			if feedbackInfo.concluded ~= true or canConclude then
				...
				exports['qb-Notify']:Alert("FEEDBACK", "Feedback #"..feedbackID.." concluded!", 20000, 'success')
			end
		end
	end
end)